...
#
# DISARM: touch $H/STOP-JACKOD-MANUAL before launch, or $H/STOP-NATIVE between
# runs.
set -uo pipefail
H="/srv/dev-disk-by-uuid-92295e2c-12bd-4d15-a50c-1d80e1a33ee8/spool/manic-harness"
LOG="$H/jackod4ac-manual-e2g-$(date '+%Y%m%d-%H%M%S').log"
E="http://192.168.178.161:11434"
V="$H/verdicts-native.txt"
# TAG swapped 2026-09-14: the jackod4ac-* tags are no longer on eleven2go --
# `jackod-9b_tb:q6_k-128k` is the only jackod tag there now, and check-tag.sh
# passes it (template 13, RENDERER/PARSER qwen3.5, think_budget present). The
# plugin sampler below is unchanged, and it is what actually decides the
# sampler, so the effective configuration is the same one the t08 runs had.
TAG="jackod-9b_tb:q6_k-128k"
RUNS="${RUNS:-1}"
export CLI_DIR=/shared/dev/cline-scan/apps/cli
# The plugin sampler is gone from here because it is in the tag now.
#
# These six fields were the de facto sampler for every jackod arm -- the plugin
# writes settings.sampling, which wins field by field over the tag's PARAMETER
# lines -- and on 2026-09-14 they were made the tag's own. Setting them here as
# well would only hide whether the tag actually carries them, so an unset
# OLLAMA_SAMPLING is now the point: what runs is the model's own sampler, and
# the gate below is what proves it is the right one.
#
#   temperature 0.8  min_p 0.05  repeat_last_n 96  repeat_penalty 1.05
#   presence_penalty 0  think_budget high  (+ top_k 20, top_p 0.95 from before)
#
# The previous value, if it ever has to come back:
...
say "=== jackod4ac MANUAL arm (atomic off) x${RUNS} on eleven2go starting ==="
say "expert=${EXPERT_MODEL:-none}"
[ -f "$H/STOP-JACKOD-MANUAL" ] && die "STOP-JACKOD-MANUAL present"
...
# The sampler, gated on the tag rather than on a variable. With the plugin
# override gone the tag is the only source, and a tag that quietly lost a
# PARAMETER line would otherwise run at ollama's defaults and say nothing --
# which is the failure this whole file's other gates exist to catch.
...
rm -f "$H/STOP-NATIVE"
say "=== ${RUNS} manual run(s): $TAG on $E, expert=${EXPERT_MODEL:-none} ==="
