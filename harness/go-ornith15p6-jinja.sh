#!/usr/bin/env bash
# 10 oracle runs of ornith15-p6-jinja-bare_tb:27b-high on eleven2go.
#
...
# num_gpu 99, temperature 0.6, top_p 0.95, top_k 20, Q4_K_M, 26.2B qwen35moe.
#
# DIFFERS FROM THE mtp ARM IN TWO PLACES AT ONCE, deliberately: this tag has NO
# RENDERER/PARSER (it runs the GGUF's own 7764-char Jinja template, which does
# emit think tags) and think_budget is high=65536 rather than medium=32768. A
# difference against the mtp numbers therefore cannot be attributed to either
# one alone. Harness is cline 4.100.80, which carries the non-convergence guard
# the mtp arm (4.100.73) did not have.
set -uo pipefail
H="/srv/dev-disk-by-uuid-92295e2c-12bd-4d15-a50c-1d80e1a33ee8/spool/manic-harness"
LOG="$H/ornith15p6-jinja.log"
E="http://192.168.178.161:11434"
TAG="ornith15-p6-jinja-bare_tb:27b-high"
