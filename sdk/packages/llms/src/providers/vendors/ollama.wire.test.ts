...
async function streamThroughVendor({
	...
	// Only the chat call. This vendor primes the model's declared `num_ctx`
	// from `/api/show` before the first request, and that POST would otherwise
	// be counted here -- invisibly, since its result is cached and only the
