...
# tool: check_file
Check files for errors and warnings using the IDE's language servers (LSP). **This is the linter** — and the type checker, and the IDE's Problems panel. Whatever the question calls it, ask here. These are live and follow your edits: a result is current as of the moment you ask, so a problem still reported after an edit is still there. There is no language server to restart from here. Call this before running a checker yourself with `run_commands`. For files the IDE understands, it answers the same question as `tsc`, `eslint`, `biome`, `ruff`, `mypy`, `go build`, or `cargo check` — for the files you name, in milliseconds, without building the project.
...
Actions: `open` (with `url`; an absolute file path is accepted and converted), `click` (with `coordinate` as `"x,y"`), `type` (with `text`), `scroll_down`, `scroll_up`, `close`.
