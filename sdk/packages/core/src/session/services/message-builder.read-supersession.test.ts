...
import { getMessageBuilderOptionsFromEnv, MessageBuilder } from "./message-builder"
 ...
 */
function readTurn(id: string, range: { start: number; end: number } | undefined, marker: string): Message[] {
			...
			content: [
				{
					...
					name: "read_files",
					input: {
						files: [range ? { path: PATH, start_line: range.start, end_line: range.end } : { path: PATH }],
					},
				},
			],
		},
			...
			content: [
				{
					...
					tool_use_id: id,
					name: "read_files",
					content: JSON.stringify([{ query, result: `${marker}-${"X".repeat(4_000)}`, success: true }]),
				},
			],
		},
	]
...
describe("read supersession", () => {
	it("keeps a ranged read that came after a whole-file read", () => {
		...
		const built = buildEagerly([...readTurn("read_full", undefined, "FULL"), ...readTurn("read_range", { start: 29, end: 136 }, "RANGE")])
	...
	it("drops a ranged read once the whole file is read again", () => {
		const built = buildEagerly([...readTurn("read_range", { start: 29, end: 136 }, "RANGE"), ...readTurn("read_full", undefined, "FULL")])
	...
	it("drops a ranged read contained by a later, wider range", () => {
		const built = buildEagerly([...readTurn("read_inner", { start: 50, end: 100 }, "INNER"), ...readTurn("read_outer", { start: 1, end: 200 }, "OUTER")])
	...
	it("keeps a ranged read that a later, narrower range does not contain", () => {
		const built = buildEagerly([...readTurn("read_outer", { start: 1, end: 200 }, "OUTER"), ...readTurn("read_inner", { start: 50, end: 100 }, "INNER")])
	...
	it("keeps ranges of different files apart", () => {
		const other: Message[] = [
			{
				role: "assistant",
				content: [{ type: "tool_use", id: "read_other", name: "read_files", input: { files: [{ path: "other.html" }] } }],
			},
			{
				role: "user",
				content: [
					{
						...
						name: "read_files",
						content: JSON.stringify([{ query: "other.html", result: `OTHER-${"X".repeat(4_000)}`, success: true }]),
					},
				],
			},
		]
		const built = buildEagerly([...readTurn("read_range", { start: 29, end: 136 }, "RANGE"), ...other])
...
describe("environment-driven rewrite threshold", () => {
	it("falls back to the default when the variable is unset", () => {
		expect(getMessageBuilderOptionsFromEnv({}).minOutdatedRewriteBytes).toBeUndefined()
	})
	...
	it("treats 'disable' as an off switch rather than a number", () => {
		...
		const built = JSON.stringify(
			builder.buildForApi([...readTurn("read_a", undefined, "FIRST"), ...readTurn("read_b", undefined, "SECOND")]) as Message[],
		)
