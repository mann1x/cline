...
import { agentMessagesToMessages, messagesToAgentMessages } from "../../runtime/config/agent-message-codec"
	...
	return [
		{
			role: "assistant",
			content: [
				{ type: "tool_use", id: `read_${n}`, name: "read_files", input: { files: [{ path: "manic_miner.html" }] } },
			],
		},
		{
			role: "user",
			content: [
				{
					...
					tool_use_id: `read_${n}`,
					name: "read_files",
					content: JSON.stringify([{ query: "manic_miner.html", result: body, success: true }]),
				},
			],
		},
		{
			...
			content: [
				{
					...
					id: `edit_${n}`,
					name: "editor",
					input: { path: "manic_miner.html", start_line: 94, end_line: 96, new_text: "x" },
				},
			],
		},
		{
			...
			content: [
				{
					...
					content: JSON.stringify([
						{ query: "edit:manic_miner.html", result: `Replaced lines 94-96 (rev ${n})`, success: true },
					]),
				},
			],
		},
...
describe("repeated whole-file reads with edits in between", () => {
	it("collapses superseded copies even though the file changed each time", () => {
		...
		for (let i = 0; i < 12; i++) messages.push(...turn(i, `rev${i}-${"X".repeat(15_000)}`))

		const builder = new MessageBuilder()
		const roundTripped = agentMessagesToMessages(messagesToAgentMessages(messages) as never) as Message[]
		const built = builder.buildForApi(roundTripped) as Message[]

		const copies = countFullCopies(built)
		const placeholders = (JSON.stringify(built).match(/outdated/g) || []).length
		console.log(`full copies still present: ${copies} of 12`)
		console.log(`outdated placeholders    : ${placeholders}`)
		console.log(`total built chars        : ${JSON.stringify(built).length.toLocaleString()}`)
