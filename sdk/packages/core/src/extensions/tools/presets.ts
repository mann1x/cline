...
export const ToolPresets = {
	...
	plan: {
		...
		enableApplyPatch: false,
		enableEditor: false,
		// grep and awk cannot write. `sed` can, so it is enabled for its
		// preview mode only: the plan-mode guard refuses `in_place: true`,
		// exactly as it refuses `sed -i` through the shell.
		enableGrep: true,
		enableSed: true,
		enableAwk: true,
		enableSkills: true,
	...
	search: {
		...
		enableApplyPatch: false,
		enableEditor: false,
		// No shell here and no guard registered, so the writer stays off.
		// grep and awk are read-only by construction.
		enableGrep: true,
		enableSed: false,
		enableAwk: true,
		enableSkills: false,
