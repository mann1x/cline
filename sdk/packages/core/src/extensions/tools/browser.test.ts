...
function result(
	overrides: Partial<BrowserActionResult> = {},
): BrowserActionResult {
	return {
		logs: "",
		currentUrl: "file:///repo/game.html",
		screenshot: PNG,
		...overrides,
...
describe("toNavigableUrl", () => {
	it("leaves a real URL alone", () => {
		expect(toNavigableUrl("http://localhost:5173/index.html", CWD)).toBe(
			"http://localhost:5173/index.html",
		);
		expect(toNavigableUrl("file:///c:/x/game.html", CWD)).toBe(
			"file:///c:/x/game.html",
		);
	});
	...
	it("gives a bare host and port the scheme it is missing", () => {
		...
		expect(toNavigableUrl("127.0.0.1:8080/app", CWD)).toBe(
			"http://127.0.0.1:8080/app",
		);
	});

	it("converts a Windows path, backslashes and all", () => {
		expect(toNavigableUrl("C:\\Users\\manni\\game.html", CWD)).toBe(
			"file:///C:/Users/manni/game.html",
		);
	});

	it("converts a POSIX path", () => {
		expect(toNavigableUrl("/repo/game.html", CWD)).toBe(
			"file:///repo/game.html",
		);
	});

	it("resolves a relative path against the working directory", () => {
		expect(toNavigableUrl("game.html", "/repo/src")).toBe(
			"file:///repo/src/game.html",
		);
	});

	it("escapes a path with a space in it", () => {
		expect(toNavigableUrl("/repo/my game.html", CWD)).toBe(
			"file:///repo/my%20game.html",
		);
	});
});
...
describe("renderBrowserResult", () => {
	...
	it("counts the errors it found", () => {
		const text = renderBrowserResult(
			"open",
			result({
				logs: "[Page Error] SyntaxError: Unexpected token ')'\nlevel loaded\n[error] boom",
			}),
		);
	...
	it("says so when there were messages but no errors", () => {
		expect(
			renderBrowserResult("open", result({ logs: "level loaded" })),
		).toContain("1 message(s), no errors");
	});

	it("names where it ended up, not only where it was sent", () => {
		const text = renderBrowserResult(
			"open",
			result({ currentUrl: "http://localhost:3000/login" }),
			"http://localhost:3000",
		);
...
describe("splitDataUrl", () => {
	it("separates the media type from the payload", () => {
		expect(splitDataUrl(PNG)).toEqual({
			mediaType: "image/png",
			data: "QkFTRTY0REFUQQ==",
		});
	});
...
describe("localPathOf", () => {
	it("unwraps a Windows file URL", () => {
		expect(localPathOf("file:///C:/Users/manni/game.html")).toBe(
			"C:/Users/manni/game.html",
		);
	});

	it("unwraps a POSIX file URL and decodes escapes", () => {
		expect(localPathOf("file:///repo/my%20game.html")).toBe(
			"/repo/my game.html",
		);
	});
...
describe("the delimiter scan on a parse error", () => {
	...
	function toolFor(logs: string, files: Record<string, string> = {}) {
		return createBrowserTool({
			cwd: CWD,
			createDriver: () =>
				fakeDriver({ navigateToUrl: vi.fn(async () => result({ logs })) }),
	...
	it("names the opener the browser could not", async () => {
		...
		const tool = toolFor(
			"[Page Error] SyntaxError: missing ) after argument list",
			{
				"/repo/game.html":
					"<body><script>\nfoo.forEach(e=>{if(e){bar();}}});\n</script></body>",
			},
	...
	it("stays quiet when the page threw something that is not a parse error", async () => {
		const tool = toolFor("[Page Error] TypeError: x is not a function", {
			"/repo/game.html": BROKEN,
		});

		expect(
			await run(tool, { action: "open", url: "/repo/game.html" }),
		).not.toContain("Delimiter scan");
	});
...
describe("browser", () => {
	...
	it("attaches a screenshot for a model that can see it", async () => {
		...
		const output = await run(
			tool,
			{ action: "open", url: "/repo/game.html" },
			true,
		);
	...
	it("rejects a coordinate that is not one", async () => {
		...
		expect(
			await run(tool, { action: "click", coordinate: "the button" }),
		).toContain('"x,y"');
	});
	...
	it("says the browser was not open rather than erroring", async () => {
		...
		expect(await run(tool, { action: "close" })).toBe(
			"The browser was not open.",
		);
	});
