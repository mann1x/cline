...
describe("createContextCompactionPrepareTurn", () => {
	...
	it("skips with a diagnostic warning when the summarizer only produced reasoning output", async () => {
		...
		// Agentic declines, but the turn is not left uncompacted: basic
		// compaction needs no model request and cannot decline for any of the
		// reasons agentic just did, so it is what runs instead. The point of this
		// test is that the decline stays diagnosable, and it does -- both the
		// reason and the fallback are logged.
		expect(result?.messages).toBeDefined();
