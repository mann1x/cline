The first public release since 4.100.118. Builds 4.100.119 to 4.100.194 were
...
- **Seed and temperature per agent.** `spawn_agent`, teammates and swarms take
  an optional `seed` and `temperature` for the agents they start.
  <!-- seed/temp: confirm wording when landed -->
...
- **Swarms are a mode of `spawn_agent`.** Set `merge: true` to run the agents on
  ...
  Where `spawn_agent` isn't available, the same thing is the `spawn_swarm`
  tool.
...
## Delegated agents work on a private copy of your workspace

- **Copy-on-write isolation.** A `spawn_agent` agent reads through to your
  workspace, but its writes, deletes and renames stay in a private overlay.
  When it finishes, each changed file comes back to the lead as a revision,
  which the lead inspects and adopts with `restore_file`. Nothing is applied
  automatically. This is always on for `spawn_agent` agents, on every
  platform.
- **Agents can run commands** (Features, off by default). An agent's commands
  run against its own copy of the workspace through a native sandbox: user
  namespaces and overlayfs on Linux, with a ptrace fallback where overlayfs
  can't run; APFS clonefile on macOS; Detours injection on Windows. All six
  launchers ship in the `.vsix`. Where no launcher exists for your platform, an
  agent gets no shell rather than an unsandboxed one.
- **Teammates, configured agents and swarm workers aren't sandboxed yet.** They
  run without an overlay, so they get no `run_commands` at all. Their file
  edits still go straight to your workspace.
...
- **Compaction Council** (on by default, three extra model calls) checks each
  half of the transcript against the summary and corrects it. Its three prompts
  are editable in Features. A prompt template can carry its own compaction
  prompts in `# compaction: <id>` sections, and **Translate Compaction
  Prompts** can generate them.
