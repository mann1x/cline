...
export function addRootOptions(cmd: Command): Command {
	return (
		cmd
			...
			.option(
				"--agent-node <spec>",
				"Where delegated agents run, as model=<id>[,url=<baseUrl>][,provider=<id>][,priority=1-10][,capacity=<n>|auto][,window-share=0-100]. Repeatable, up to ten. capacity=auto imposes no ceiling and lets the endpoint's own admission control refuse. window-share (opencoti only, default 50) is the least window an agent accepts, from 0 (the minimum a turn needs: system prompt, tools and output cap) to 100 (the node's whole window). Priority 1 is highest and a lower tier is used only when nothing above it has a free slot; round-robin within a tier; every node full means the next agent waits rather than fails",
