...
const CLINE_OUTPUT_CHANNEL = vscode.window.createOutputChannel("Cerebriline")

// Appends a log message to all Cerebriline output channels.
export async function debugLog(request: StringRequest): Promise<Empty> {
	CLINE_OUTPUT_CHANNEL.appendLine(request.value)
	return Empty.create({})
}

// Register the Cerebriline output channel within the VSCode extension context.
