 ...
 * exercising and debugging Cerebriline's MCP OAuth flow locally, including failure
 * modes such as:
 *
 ...
 *                         for exercising Cerebriline's OAuth state-expiry window).
 ...
 *
 * Then add an MCP server to Cerebriline pointing at:
 *   http://127.0.0.1:7777/mcp   (type: streamableHttp)
 *
 * Click "Authenticate" in Cerebriline; a browser opens the /authorize consent page.
 */
...
interface TestServerOptions {
	 ...
	 * so you can add several distinct MCP servers to Cerebriline at once and exercise
...
Options:
  ...
  --instances <n>       Start N independent servers, each on its own random
                        ...
                        servers to Cerebriline at once.
...
class TestServer {
	...
	private async handleMcp(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (!hasBearer) {
			// This is what triggers Cerebriline's OAuth flow: 401 + WWW-Authenticate
		...
		// succeeds and Cerebriline shows the server as connected.
		const body = await this.readJsonBody(req)
