...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		...
		if (request.agentNodes !== undefined) {
			controller.stateManager.setGlobalState("agentNodes", request.agentNodes)
		}
		...
		if (request.escalationSettings !== undefined) {
			...
			const struggle = {
				...
				struggleFailedTransactions: request.escalationSettings.struggleFailedTransactions,
			}
