...
export const CODE_INTEL_TOOL_INPUT_SCHEMA = {
	...
	properties: {
		...
		path: {
			type: "string",
			description: "File the symbol appears in. Absolute, or relative to the working directory.",
		},
		symbol: {
			type: "string",
			description: "The symbol's name, as written in that file. The usual way to address a symbol.",
		},
		line: { type: "number", description: "1-based line, if you know the exact position." },
		character: { type: "number", description: "1-based column, if you know the exact position." },
	},
	required: ["operation"],
} as const
...
export interface CodeIntelProvider {
	...
	findSymbolPosition(filePath: string, symbol: string): Promise<CodeIntelLocation | undefined>
 ...
 * interpret.
 */
export function parseCodeIntelRequest(input: CodeIntelInput | undefined): ParsedCodeIntelRequest | string {
	const operation = readString(input?.operation)?.toLowerCase() as CodeIntelOperation | undefined
	...
	if (operation === "workspace_symbols") {
		return symbol ? { operation, symbol } : { operation, symbol: filePath ?? "" }
	}
	if (operation === "document_symbols") {
		return filePath ? { operation, filePath } : "`document_symbols` needs a `path`."
	}
	if (!filePath) {
...
export function createCodeIntelTool(options: CodeIntelToolOptions): AgentTool {
	...
	const renderLocations = async (locations: readonly CodeIntelLocation[], empty: string): Promise<string> => {
		...
		const lines = await Promise.all(
			shown.map(async (location) => {
				const source = (await provider.readLine(location.filePath, location.line))?.trim()
		...
		return omitted > 0 ? `${lines.join("\n")}\n…and ${omitted} more` : lines.join("\n")
	}

	const renderSymbols = (symbols: readonly CodeIntelSymbol[], empty: string): string => {
		if (symbols.length === 0) {
			return empty
		}
	...
	return createTool({
		...
		execute: async (rawInput: unknown) => {
			const request = parseCodeIntelRequest(rawInput as CodeIntelInput | undefined)
			...
			try {
				...
				if (request.operation === "document_symbols") {
					return renderSymbols(await provider.documentSymbols(filePath), `${display}: no symbols reported.`)
				}
				...
				switch (request.operation) {
					case "definition":
						return await renderLocations(await provider.definitions(at), `No definition found for that symbol.`)
					case "type_definition":
						return await renderLocations(await provider.typeDefinitions(at), `No type definition found.`)
					case "implementations":
						return await renderLocations(await provider.implementations(at), `No implementations found.`)
					case "references":
						return await renderLocations(await provider.references(at), `No references found.`)
					case "callers":
						return renderSymbols(await provider.callers(at), `Nothing calls that.`)
					case "hover": {
						const hover = await provider.hover(at)
						return hover?.trim() ? hover.trim() : `The language server had nothing to say about that symbol.`
					}
					default:
