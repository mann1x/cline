...
describe("RealtimeVoiceOverlay", () => {
	it("acquires the system microphone and waits to capture until the greeting finishes", async () => {
		await act(async () => {
			root.render(
		...
		expect(container.textContent).toContain("Input: Studio Microphone · ready");
		expect(mocks.startAudioCapture).not.toHaveBeenCalled();
	});

	it("greets the user after the realtime connection is ready", async () => {
		const props = {
			bridge: makeBridge(),
			onConfigure: vi.fn(),
			onOpenChange: vi.fn(),
		...
		expect(mocks.speakGreeting).toHaveBeenCalledOnce();
		expect(
			(mocks.speakGreeting.mock.calls[0]?.[0] as { text?: string } | undefined)
				?.text,
		).toBe("Hi! I'm Cline. Your voice session is ready. How can I help you?");
		expect(mocks.sendEvent).not.toHaveBeenCalled();
		expect(props.bridge.sendPrompt).not.toHaveBeenCalled();
		expect(mocks.stopAudioCapture).not.toHaveBeenCalled();
		expect(mocks.startAudioCapture).not.toHaveBeenCalled();

		const utterance = mocks.speakGreeting.mock.calls[0]?.[0] as
			| { onend?: (() => void) | null }
			| undefined;
		act(() => utterance?.onend?.());

		expect(mocks.startAudioCapture).toHaveBeenCalledOnce();
		expect(container.textContent).toContain("Input: Studio Microphone · live");
	});
	...
	it("restores microphone input when WebKit omits the greeting completion event", async () => {
		vi.useFakeTimers();
		try {
			const props = {
				bridge: makeBridge(),
	...
	it("mutes ambient turns without dropping speech completed before mute", async () => {
		...
		expect(container.textContent).toContain("Input: Studio Microphone · muted");
	...
	it("keeps a single greeting transcript when the voice session restarts", async () => {
		const greeting =
			"Hi! I'm Cline. Your voice session is ready. How can I help you?";
		const props = {
			bridge: makeBridge(),
		...
		expect(greetingEntries).toHaveLength(1);
		expect(mocks.speakGreeting).toHaveBeenCalledTimes(2);
	});
