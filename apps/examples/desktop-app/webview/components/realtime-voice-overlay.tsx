...
const READY_GREETING_INPUT_RESUME_TIMEOUT_MS = 6_000;
const REALTIME_READY_GREETING =
...
function ConfiguredRealtimeVoiceOverlay({
	...
	const [playingReadyGreeting, setPlayingReadyGreeting] = useState(false);
	const [hearingSpeech, setHearingSpeech] = useState(false);
	const streamRef = useRef<MediaStream | null>(null);
	const readyGreetingUtteranceRef = useRef<SpeechSynthesisUtterance | null>(
		null,
	);
	const readyGreetingResumeTimeoutRef = useRef<number | null>(null);
	...
	const stopMedia = useCallback(() => {
		if (readyGreetingResumeTimeoutRef.current !== null) {
			window.clearTimeout(readyGreetingResumeTimeoutRef.current);
			readyGreetingResumeTimeoutRef.current = null;
		}
		if (readyGreetingUtteranceRef.current) {
			readyGreetingUtteranceRef.current = null;
	...
	const speakReadyGreeting = useCallback(() => {
		if (
			...
			writeDesktopDebugLog({
				...
				level: "warn",
				message:
					"Browser speech synthesis is unavailable for the ready greeting",
				timestamp: new Date().toISOString(),
		...
		// Do not attach capture until this local greeting finishes. This prevents
		// the realtime model from hearing its own greeting without muting or
		// stopping a live WebKit MediaStream.
		const utterance = new SpeechSynthesisUtterance(REALTIME_READY_GREETING);
		setPlayingReadyGreeting(true);
		const resumeCapture = () => {
			if (readyGreetingUtteranceRef.current !== utterance) return;
			if (readyGreetingResumeTimeoutRef.current !== null) {
				window.clearTimeout(readyGreetingResumeTimeoutRef.current);
				readyGreetingResumeTimeoutRef.current = null;
			}
		...
		utterance.onend = resumeCapture;
		utterance.onerror = (event) => {
			writeDesktopDebugLog({
				scope: "realtime-voice",
				level: "warn",
				message: "Browser speech synthesis failed for the ready greeting",
				timestamp: new Date().toISOString(),
				metadata: {
					providerId,
					modelId,
		...
		readyGreetingUtteranceRef.current = utterance;
		readyGreetingResumeTimeoutRef.current = window.setTimeout(() => {
			writeDesktopDebugLog({
				scope: "realtime-voice",
				level: "debug",
				message:
					"Ready greeting completion event timed out; restoring microphone input",
				timestamp: new Date().toISOString(),
				metadata: { providerId, modelId },
			});
			resumeCapture();
		}, READY_GREETING_INPUT_RESUME_TIMEOUT_MS);
		try {
			...
			writeDesktopDebugLog({
				scope: "realtime-voice",
				level: "warn",
				message: "Browser speech synthesis could not start the ready greeting",
				timestamp: new Date().toISOString(),
				metadata: {
					providerId,
					modelId,
	...
	}, [modelId, providerId, startMicrophoneCapture]);
	...
	useEffect(() => {
		...
		if (bridge?.hasChatHistory) {
			startMicrophoneCapture();
			return;
		}
		appendTranscript({
			id: "assistant:ready-greeting",
			role: "assistant",
			text: REALTIME_READY_GREETING,
		});
		speakReadyGreeting();
	}, [
		appendTranscript,
		bridge?.hasChatHistory,
		speakReadyGreeting,
		startMicrophoneCapture,
		status,
	]);
	...
	return (
		<div className="relative flex shrink-0 items-center gap-1">
			...
			>
				<div className="flex items-start gap-3 border-b border-border px-3 py-2.5">
					...
					<div className="min-w-0 flex-1">
						<div className="flex items-center gap-1.5 text-sm font-medium">
							{isPlaying ? <Volume2 className="size-3.5" /> : null}
							<span>{statusLabel}</span>
						</div>
						<div className="truncate text-xs text-muted-foreground">
							Voice: {providerName} / {modelName}
						</div>
						<div
						...
						>
							Input:{" "}
							{microphone
								? `${microphone.label} · ${microphone.state}`
								: "System default microphone · requesting"}
						</div>
						{bridge ? (
							<div className="truncate text-[11px] text-muted-foreground">
								Agent: {bridge.providerId} / {bridge.modelId}
							</div>
						) : null}
					</div>
					<button
				...
				<div className="max-h-52 space-y-2 overflow-y-auto px-3 py-2.5 text-xs">
					{transcript.length > 0 ? (
						...
						<p className="text-muted-foreground">
							{status === "connected"
								? isCapturing
									? "Your microphone is live. Speak to the same Cline agent as this chat."
									: "Finishing the greeting before microphone capture starts…"
