<#
.SYNOPSIS
    Collects Cerebriline session transcripts and logs into a zip for analysis.

    For an installation that has NOT been migrated off the old names, use
    Collect-ClineReport-PreMigration.ps1 instead. This one understands both
    layouts, but that one is the known-good collector for the old one.
...
.EXAMPLE
    .\Collect-CerebrilineReport.ps1

    Lists the sessions and asks which to include.

.EXAMPLE
    .\Collect-CerebrilineReport.ps1 -Latest

    Takes the most recent session without asking.

.EXAMPLE
    .\Collect-CerebrilineReport.ps1 -SessionCount 3 -OutputPath C:\temp

.EXAMPLE
    .\Collect-CerebrilineReport.ps1 -StripImages
...
$reportId = "cerebriline-report-$env:COMPUTERNAME-$stamp"
...
# Which home data directory the extension is actually reading.
#
# It prefers `.cerebriline` and falls back to `.cline` when only that exists,
# so the collector has to make the same choice or it collects a directory
# nothing is writing to. Both are reported either way: an install with both
# present is the split that produces "it stopped seeing my history", and
# naming it here is how that becomes visible instead of being guessed at.
$dataRoots = @(
    (Join-Path $env:USERPROFILE '.cerebriline'),
    (Join-Path $env:USERPROFILE '.cline')
)
$presentRoots = @($dataRoots | Where-Object { Test-Path -LiteralPath $_ })
$dataRoot = if ($presentRoots.Count -gt 0) { $presentRoots[0] } else { $dataRoots[0] }

if ($presentRoots.Count -gt 1) {
    Add-Note "Both data directories exist; using $dataRoot (the one the extension prefers)." 'Yellow'
    Add-Note "The other is $($presentRoots[1]) - migration may be half done." 'Yellow'
}

$sessionsRoot = Join-Path $dataRoot 'data\sessions'

if (-not (Test-Path -LiteralPath $sessionsRoot)) {
    Add-Note "No session directory at $sessionsRoot." 'Red'
    Add-Note "Looked for both .cerebriline and .cline under $env:USERPROFILE." 'Red'
    Add-Note "Has Cerebriline been run on this machine as this user?" 'Red'
    exit 1
}
...
# The file is named after the output channel, and the channel was renamed with
# the extension: `Cline` became `Cerebriline`, so a collector that globs only
# the old name finds nothing and reports no logs at all. Measured on a real
# report: a 4.100.118 install produced a zip with no extension log in it, which
# is exactly the evidence the report existed to carry. Both names are taken,
# because one machine can have logs from windows opened either side of the
# upgrade.
$clineLogs = @()
foreach ($root in $logRoots) {
    foreach ($pattern in @('*-Cerebriline.log', '*-Cline.log')) {
        $clineLogs += Get-ChildItem -LiteralPath $root -Recurse -Filter $pattern -ErrorAction SilentlyContinue
    }
}
...
if ($nonEmpty.Count -gt 0) {
    ...
    Add-Missing 'logs' 'every *-Cerebriline.log / *-Cline.log found was empty'
} else {
    Add-Missing 'logs' 'no *-Cerebriline.log or *-Cline.log found under any VS Code logs directory'
}

$hooksLog = Join-Path $dataRoot 'data\logs\hooks.jsonl'
...
if (-not $NoSettings) {
    $settingsRoot = Join-Path $dataRoot 'data\settings'
...
if (Test-Path -LiteralPath $extRoot) {
    $extensionDirs = @(Get-ChildItem -LiteralPath $extRoot -Directory |
        Where-Object {
            # `cerebriline` does not contain `cline`, so the old two patterns
            # match neither the current extension id nor its folder, and the
            # report then claims nothing is installed.
            $_.Name -like '*cerebriline*' -or $_.Name -like '*claude-dev*' -or $_.Name -like '*cline*'
        } |
